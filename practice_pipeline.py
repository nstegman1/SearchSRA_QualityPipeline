...
if args.entrez_email is None:
    parser.error('Email for eztrez esearch must be provided.')
...
log_file = open(base_path+'/sra_pipeline_results.csv', 'w+')
...
def parse_pileup_files(cwd):
    ...
    log_file.write('SRA Number'+'\t'+'Query Coverage %'+'\t'+'Average Read Count'+'\t'+'Study Title'+'\n')
    ...
    for i,j in reversed(filtered_coverage):
        ...
        #if it errors, go to the next file, and writ it out as data not avaliable 
        #do a try statement, then exception if it errors. 
